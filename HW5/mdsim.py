...
#def of print the momentum
def plot_moment(momen_all,nsteps,dt,temperature):
    plt.figure('Momentum')
    xx = [dt*i for i in range(nsteps)]
    plt.plot(xx,momen_all[:,0],label='x-direction')
    plt.plot(xx,momen_all[:,1],label='y-direction')
    plt.plot(xx,momen_all[:,2],label='z-direction')
    plt.legend()
    plt.xlabel('time')
    plt.ylabel('momentum')
    plt.title('Plot momentum at T = %s'%str(temperature))
    plt.savefig('momentum_T_%s.png'%str(temperature))
    plt.show()
#end def
...
def plot_pair_corr(r_num,g_r_ave,box_length,temperature):
    rstep = box_length/r_num/2
    r_i = np.zeros(r_num)
    for i in range(r_num):
        r_i[i] = (i+0.5)*rstep
    ...
    plt.title('Plot of pair correlation g(r) at T = %s'%str(temperature))
    plt.savefig('Pair_corr_T_%s.png'%str(temperature))
    plt.show()
...
def plot_str_factor(s_r_ave,maxk,box_length,temperature,ndim):
    kvecs   = legal_kvecs(maxk,ndim,box_length)
    #average sk and plot the final results
    kmags  = [np.linalg.norm(kvec) for kvec in kvecs]
    #average s(k) if different k-vectors share the same magnitude
    ...
    plt.title('s(k) when kmax = %d and T = %f'%(maxk,temperature))
    plt.savefig('Str_fac_T_%s.png'%str(temperature))
    plt.show()
...
if __name__ == '__main__':
    ...
    mass        = 48.0
    dt          = 0.032      # time step
    temperature = 0.1   # initla temperature
    ...
    for istep in range(nsteps):
        ...
        if istep == 0:
            ...
            for i in range(num_atoms):
                vv_0[i] = np.dot(veloc_t0[i],veloc_t0[i])
        #end if
        ...
        su_jj = 0.0
        for i in range(num_atoms):
            su_jj += np.dot(veloc_t0[i],veloc_t[i])/vv_0[i]
        #end for
        ...
        #calculate the momentum for each step
        momen[istep] = momentum(pset.all_vel(),mass)
        ...
        for iat in range(num_atoms):
            new_accel[iat] = internal_force(iat,pset,box_length)/mass
        #end for
        ...
        if istep >firstcutoff:  # take the average value of g(r)
            ...
            s_r_ave += 1.0/(nsteps-firstcutoff)*structure_r(pset,maxk,box_length)
        #end if
    #end for loop
    ...
    #plot the momentum to see whether its is conserved or not
    plot_moment(momen,nsteps,dt,temperature)

    # do pair correlation calculation and plot
    plot_pair_corr(r_num,g_r_ave,box_length,temperature)


    #----------calculation and plot for structure factors s(k) -----------
    plot_str_factor(s_r_ave,maxk,box_length,temperature,3)
