...
class ParticleSet:
    ...
    def init_vel(self, temperature, seed):

        np.random.seed(seed)

        #print the information of seed on screen
        print('*'*40)
        print('seed = %d'%seed)
        print('initial random number = %.8f'%random.random())
        print('*'*40)
        # end the print of seed information
...
if __name__ == '__main__':
    ...
    print(Uinternal)
    assert abs(tot_kinetic-Uinternal) < 1e-10, "total kinetic energy = %f is different from internal energy" % tot_kinetic
