...
class SimulationCube:
    ...
    def __init__(self,cube_length,pset):
        ...
        self._pset = pset # keep a REFERENCE to pset for speed, do NOT edit internals of pset in this class!
    ...
    def update_displacement_table(self):
        ...
        for iat in range(natom):
            for jat in range(iat+1,natom):
                self._dist_table[iat,jat] = np.linalg.norm(self._disp_table[iat,jat])
                # lower triangular part of distance matrix is not calculated
                #self._dist_table[jat,iat] = self._dist_table[iat,jat]
            # end for
        # end for
        """ # vectorized version is slower?
        self._dist_table[:,:] = np.apply_along_axis(np.linalg.norm,1
            ,self._disp_table.reshape(self._pset.size()**2,self._pset.ndim())
        ).reshape(self._pset.size(),self._pset.size())
        """
